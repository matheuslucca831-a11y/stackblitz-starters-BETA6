  ...
  const gerarExcel = async () => {
    ...
    categorias.forEach((cat) => {
      ...
      const itensDaCategoria = itensCatalogo.filter(item => {
        ...
        if (cat.chave === 'CORRELATOS') return itemCat === 'CORRELATOS' || itemCat === 'CORRELATOS - PACIENTES CRÔNICOS/ ACAMADOS';
      ...
      itensDaCategoria.forEach((item) => {
        ...
        row.getCell(2).value = item.id;    // Código oficial do material
