...
import { LISTA_LIMPEZA as listaInicial } from '../produtos';
import { saveAs } from 'file-saver';
import * as docx from 'docx';
import { FileText, Search, Plus, Trash2, Layout } from 'lucide-react';
import { logoFSPSS, logoBrasil } from './imagens';

export default function PedidoLimpezaCompleto() {
  const [unidade, setUnidade] = useState('');
  const [busca, setBusca] = useState('');
  
  // Transformamos a lista em estado para permitir Adicionar/Excluir
  ...
  // 1. Função para Adicionar Novo Item na lista
  ...
  // 2. Função para Excluir Item do catálogo
  ...
  const exportarDocx = () => {
    ...
    const doc = new docx.Document({
      ...
      sections: [{
            ...
            children: [
              new docx.Table({
                ...
                rows: [
                  new docx.TableRow({
                    children: [
                      // Coluna 1: Logo FSPSS (Esquerda) - Altura 3,04 cm | Largura 2,41 cm
                      new docx.TableCell({
                      ...
                      new docx.TableCell({
                        ...
                        children: [
                          ...
                          new docx.Paragraph({
                            ...
                            spacing: { 
                              line: 360, 
                              after: 800 // Espaço extra abaixo da lei complementar
                            },
                          }),
                        ],
                        verticalAlign: docx.VerticalAlign.CENTER,
                      }),
                      // Coluna 3: Logo Brasil (Direita) - Altura 2,93 cm | Largura 2,34 cm
                      new docx.TableCell({
                        width: { size: 20, type: docx.WidthType.PERCENTAGE },
                        children: [
                          new docx.Paragraph({
        ...
        children: [
          ...
          // 2. UNIDADE E DATA (TABELA INVISÍVEL PARA ALINHAMENTO PERFEITO)
          new docx.Table({
            width: { size: 100, type: docx.WidthType.PERCENTAGE },
            borders: docx.TableBorders.NONE,
            rows: [
    ...
    docx.Packer.toBlob(doc).then(blob => saveAs(blob, `Pedido_Limpeza_${unidade}.docx`));
  };
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen text-black">
      <div className="max-w-6xl mx-auto bg-white shadow-2xl rounded-xl overflow-hidden border border-gray-200">
        ...
        {/* HEADER DA PÁGINA (ESTILO FSPSS) */}
        <div className="p-8 border-b bg-white">
          <div className="flex justify-between items-start mb-6">
             <div className="text-center w-full">
                <h1 className="text-xl font-black text-gray-800 tracking-tighter">FUNDAÇÃO DE SAÚDE PÚBLICA DE SÃO SEBASTIÃO</h1>
                <p className="text-xs italic text-gray-500">Lei Complementar nº 168/2013 e alterações</p>
                <div className="mt-4 inline-block bg-gray-900 text-white px-4 py-1 rounded text-sm font-bold uppercase tracking-widest">
                  Pedido Mensal – Material de Limpeza 2026
                </div>
             </div>
          </div>
